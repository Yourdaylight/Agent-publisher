...
def cmd_sync_article(args):
    ...
    print(
        f"{emoji} Sync finished: overall_status={overall} sync_status={sync_status} targets={targets}"
    )
    if results:
        print(f"\n  {'Account':<20} {'Status':<10} {'Media ID':<30} {'Error'}")
        print(f"  {'-' * 20} {'-' * 10} {'-' * 30} {'-' * 20}")
...
def build_parser():
    ...
    p.add_argument(
        ...
        action="append",
        dest="account_id",
        help="Target account ID (repeatable for multi-account)",
    )
    p.add_argument(
        "--test-mode", action="store_true", help="Enable test mode: print full JSON response"
    )

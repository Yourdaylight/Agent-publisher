...
def cmd_accounts(args):
    ...
    if not result:
        ...
        return
    for acc in result:
        print(f"  [{acc['id']}] {acc['name']}  appid={acc['appid']}  owner={acc.get('owner_email', 'N/A')}")
...
def cmd_create_account(args):
    ...
    print(f"✅ Account created: id={result['id']} name={result['name']} owner={result.get('owner_email')}")
...
def cmd_agents(args):
    ...
    for ag in result:
        ...
        print(f"  {status} [{ag['id']}] {ag['name']}  topic={ag['topic']}  account_id={ag['account_id']}  style={style}  cron={ag.get('schedule_cron', 'N/A')}")
...
def cmd_create_agent(args):
    ...
    if args.rss:
        payload['rss_sources'] = [{'name': r.split('=')[0], 'url': r.split('=')[1]} for r in args.rss]
    if args.image_style:
...
def cmd_articles(args):
    ...
    if not result:
        ...
        return
    for art in result:
        print(f"  [{art['id']}] {art.get('title', 'Untitled')}  status={art.get('status')}  agent_id={art.get('agent_id')}")
...
def cmd_publish(args):
    ...
    payload = {}
    if args.account_id:
        payload['target_account_ids'] = args.account_id
    result = api(
        'POST',
        f'{base}/api/skills/articles/{args.article_id}/publish',
        token=token,
        json=payload if payload else None,
    )
    ok = result.get('ok', False)
    overall = result.get('overall_status', 'unknown')
    targets = result.get('target_account_ids', [])
    results = result.get('results', [])
    emoji = '✅' if ok else '❌'
    print(f"{emoji} Publish finished: overall_status={overall} targets={targets}")
    if results:
        print(f"\n  {'Account':<20} {'Status':<10} {'Media ID':<30} {'Error'}")
        print(f"  {'-'*20} {'-'*10} {'-'*30} {'-'*20}")
        for r in results:
            name = r.get('account_name', str(r.get('account_id', '?')))
            st = r.get('status', '?')
            mid = r.get('wechat_media_id', '-') or '-'
            err = r.get('error', '') or '-'
            st_emoji = '✅' if st == 'success' else ('⏭️' if st == 'skipped' else '❌')
            print(f"  {name:<20} {st_emoji} {st:<7} {mid:<30} {err}")
    if args.test_mode:
        print(f"\n📋 Test mode summary:")
        pp(result)
...
def cmd_create_article(args):
    ...
    print(f"✅ Article created: id={result['id']} title={result['title']} status={result['status']}")
...
def cmd_edit_article(args):
    ...
    if not payload:
        print('Error: No fields specified to update. Use --title, --digest, --content, --content-file, or --cover.', file=sys.stderr)
        sys.exit(1)
...
def cmd_sync_article(args):
    """Sync article edits to one or more WeChat draft boxes."""
    base = get_base_url(args)
    token = require_token(args)
    payload = {}
    if args.account_id:
    ...
    result = api(
        'POST',
        f'{base}/api/skills/articles/{args.article_id}/sync',
        token=token,
        json=payload if payload else None,
    )
    ok = result.get('ok', False)
    ...
    targets = result.get('target_account_ids', [])
    results = result.get('results', [])
    emoji = '✅' if ok else ('⏭️' if sync_status == 'skipped' else '❌')
    print(f"{emoji} Sync finished: overall_status={overall} sync_status={sync_status} targets={targets}")
    if results:
        print(f"\n  {'Account':<20} {'Status':<10} {'Media ID':<30} {'Error'}")
        print(f"  {'-'*20} {'-'*10} {'-'*30} {'-'*20}")
        for r in results:
...
def cmd_batch_publish(args):
    """Batch publish articles to one or more accounts (admin only)."""
    base = get_base_url(args)
    token = require_token(args)
...
def cmd_media(args):
    ...
    for m in result:
        ...
        print(f"  [{m['id']}] {m['filename']}  {size_kb:.1f}KB  type={m['content_type']}  tags=[{tags}]  url={m.get('url')}")
...
def cmd_article_stats(args):
    ...
    if article_summary:
        ...
        for item in article_summary:
            ...
            print(f"   {ref_date:<12} {int_page_read_count:>8} {share_count:>8} {add_to_fav_count:>10}")

    article_total = result.get('article_total', [])
    if article_total:
        print(f"\n   Per-Article Detail:")
        print(f"   {'Date':<12} {'Title':<30} {'Read(U)':>8} {'Read(C)':>8} {'Share(U)':>9} {'Fav(U)':>7}")
        ...
        for item in article_total:
            ...
            if details_list:
                for detail in details_list:
                    ...
                    print(f"   {stat:<12} {title:<30} {read_user:>8} {read_count:>8} {share_user:>9} {fav_user:>7}")
            else:
...
def cmd_edit_style(args):
    ...
    if not payload:
        print('Error: Specify at least one field: --name, --description, --prompt, or --prompt-file.', file=sys.stderr)
        sys.exit(1)
...
def cmd_generate_variants(args):
    ...
    result = api('POST', f'{base}/api/skills/articles/{args.article_id}/variants', token=token, json=payload)
    if result.get('ok'):
        print(f"\U0001f680 Variant generation started: batch_task_id={result.get('batch_task_id')} total={result.get('total')}")
    else:
        print(f'\u274c Failed: {result}')
        ...
        for _ in range(120):
            ...
            if status in ('completed', 'partial_completed', 'failed'):
                ...
                for st in subtasks:
                    ...
                    article_info = f" article_id={st.get('article_id')}" if st.get('article_id') else ''
                    error_info = f" error={st.get('error')}" if st.get('error') else ''
                    print(f"  {st_emoji} agent={st.get('agent_id')} style={st.get('style_id')}{article_info}{error_info}")
                return
        print('\u23f1\ufe0f  Timeout. Check task status manually.')
...
def cmd_variants(args):
    ...
    if not result:
        ...
        return
    for v in result:
        print(f"  [{v['id']}] {v.get('title', 'Untitled')}  style={v.get('variant_style', '-')}  agent={v.get('agent_name', v.get('agent_id'))}  status={v.get('status')}")
...
def build_parser():
    ...
    p.add_argument('--default-style', help='Default variant style preset ID (empty string to clear)')
    p.add_argument('--cron', help='New schedule cron')
    p.add_argument('--active', type=lambda x: x.lower() in ('true', '1', 'yes'), help='Active status (true/false)')
    ...
    p.add_argument('article_id', type=int, help='Article ID')
    p.add_argument('--account-id', type=int, action='append', dest='account_id', help='Target account ID (repeatable for multi-account)')
    p.add_argument('--test-mode', action='store_true', help='Enable test mode: print full JSON response')
    p.set_defaults(func=cmd_publish)

    # create-article
    p = sub.add_parser('create-article', help='Create article with custom content')
    p.add_argument('--agent-id', type=int, required=True, help='Agent ID (determines which account to publish to)')
    ...
    p.add_argument('--cover', help='Cover image: media:<id>, media library ID (number), or http(s) URL')
    ...
    p.add_argument('--cover', help='New cover image: media:<id>, media library ID (number), or http(s) URL')
    ...
    p.add_argument('--styles', required=True, help='Comma-separated style IDs (e.g. tech,uncle,clickbait)')
    p.add_argument('--wait', '-w', action='store_true', help='Wait for completion')

<template>
  <t-form :data="formData" :rules="rules" ref="formRef" label-width="120px" @submit="onSubmit">
    ...
    <!-- ===== 高级配置（可折叠） ===== -->
    <t-divider>
      <span style="cursor: pointer; color: var(--td-brand-color); font-size: 13px; user-select: none" @click="showAdvanced = !showAdvanced">
    ...
    <template v-if="showAdvanced">
      <t-form-item label="描述" name="description">
        <t-textarea v-model="formData.description" placeholder="Agent 的职责描述（可选）" :autosize="{ minRows: 2 }" />
      </t-form-item>
      ...
      <t-form-item label="LLM 配置">
        <t-select v-model="formData.llm_profile_id" placeholder="使用全局默认" clearable>
          <t-option v-for="p in llmProfiles" :key="p.id" :value="p.id" :label="`${p.name} — ${p.provider}/${p.model}`" />
        </t-select>
        <div style="margin-top: 4px; font-size: 12px; color: var(--td-text-color-placeholder)">
          留空将使用全局默认配置。<router-link to="/llm-profiles">管理 LLM 配置 &rarr;</router-link>
        </div>
      </t-form-item>
...
// 编辑模式默认展开高级配置，新建模式默认收起
const showAdvanced = ref(!!props.editId);
...
const formData = reactive({
  ...
  llm_profile_id: null as number | null,
  prompt_template: '',
...
onMounted(async () => {
  try {
    const [accRes, styleRes, profilesRes] = await Promise.all([getAccounts(), getStylePresets(), getLLMProfiles()]);

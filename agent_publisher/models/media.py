...
class MediaAsset(Base):
    ...
    stored_filename: Mapped[str] = mapped_column(
        String(300), unique=True, comment="UUID-based stored filename"
    )
    ...
    tags: Mapped[list | None] = mapped_column(
        JSON, default=list, comment="Tag list for categorization"
    )
    ...
    source_kind: Mapped[str] = mapped_column(
        String(50), default="manual", comment="manual / article_body / article_cover"
    )
    source_url: Mapped[str] = mapped_column(
        String(1000), default="", comment="Original remote URL or inline image fingerprint"
    )
    ...
    def __repr__(self) -> str:
        return (
            f"<MediaAsset id={self.id} filename={self.filename!r} source_kind={self.source_kind!r}>"
        )

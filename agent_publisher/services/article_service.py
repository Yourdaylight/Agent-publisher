...
class ArticleService:
        ...
        if wenyan_bin:
            try:
                ...
                if result.returncode == 0 and result.stdout.strip():
                    ...
                    # wenyan already provides comprehensive inline styles;
                    # only supplement <strong>/<em> which wenyan misses.
                    return WeChatStyleService.inject_styles_supplement(result.stdout.strip())
                logger.warning(
    ...
    async def _get_publish_html(self, article: Article) -> str:
        ...
        if not publish_html and article.content:
            # No html_content yet — render from Markdown
            logger.info(
            ...
            await self.session.flush()
        elif publish_html and not article.content:
            # HTML-only article (no Markdown source) — safety net:
            # inject WeChat inline styles into any unstyled tags.
            ...
            await self.session.flush()
        # else: has both content and html_content — html was rendered from
        # markdown (by wenyan or AI beautify), already properly styled.
    ...
    _AI_ENHANCE_SYSTEM_PROMPT = (
        "你是一位顶级微信公众号排版设计师。\n"
        "你会收到一段已经通过 wenyan 排版引擎渲染好的 HTML，它已有基本的 inline style 排版。\n\n"
        "## 你的任务\n"
        "在现有排版基础上做**增强美化**，而不是从头重写。\n\n"
        "## 规则\n"
        "1. 保留原有的 HTML 结构和 inline style，在此基础上微调\n"
        "2. 可以做的增强：\n"
        "   - 给 h2 添加装饰元素（左侧色条、底部渐变线、浅色背景色块等，选一种风格保持统一）\n"
        "   - 给 blockquote 增加圆角卡片感（调整 border-radius、background）\n"
        "   - 给关键 <strong> 文字添加高亮色（#07C160 或 #2563eb）\n"
        "   - 用装饰性分隔线替换 <hr>（居中小圆点、渐变线等）\n"
        "   - 结尾段落用小字号/不同颜色做 visual break\n"
        "   - 首段或核心段可以用大字号(17px)强调\n"
        "3. 不能做的事：\n"
        "   - 不要删除或改写正文文字内容\n"
        "   - 不要使用 <style> / <link> / class / id（微信不支持）\n"
        "   - 不要移除原有的 inline style，只能追加或微调属性值\n"
        "   - 不要改变 DOM 结构（不要把 <p> 拆成 <section> 等）\n"
        "4. 配色方案（选一套保持统一）：\n"
        "   - 方案A（微信绿）: 主色 #07C160, 辅色 #f0fdf4\n"
        "   - 方案B（科技蓝）: 主色 #2563eb, 辅色 #eff6ff\n"
        "   - 方案C（商务灰）: 主色 #374151, 辅色 #f3f4f6\n"
        "   根据文章主题自动选择最匹配的配色。\n"
        "5. 只输出 HTML，不要解释\n"
    )

    _AI_REDESIGN_SYSTEM_PROMPT = (
    ...
    async def ai_beautify_html(
        self, article: Article, style_hint: str = "", theme: str = "default"
    ) -> str:
        """Use LLM to beautify article HTML for WeChat Official Account layout.

        When the article has Markdown source, it is first rendered through wenyan
        to produce a clean styled base, then the LLM enhances that base.
        When only HTML exists (no Markdown), the LLM redesigns from scratch.

        Args:
            article: Article instance with content and/or html_content.
            style_hint: Optional style direction for the LLM.
            theme: Wenyan theme for base rendering (default, orangeheart, etc.).
        """
        from agent_publisher.config import settings

        # Determine input HTML and prompt strategy
        if article.content:
            # Markdown exists → render through wenyan first for a clean base
            html_input = self._markdown_to_html(article.content, theme=theme)
            system_prompt = self._AI_ENHANCE_SYSTEM_PROMPT
        else:
            # HTML-only → use existing html_content, full redesign prompt
            html_input = article.html_content or ""
            system_prompt = self._AI_REDESIGN_SYSTEM_PROMPT

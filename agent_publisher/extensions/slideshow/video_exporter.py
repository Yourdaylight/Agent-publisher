...
class VideoExporter:
    """Record a reveal.js auto-play presentation and produce an mp4."""
    ...
    async def export(
        ...
        try:
            ...
            if tts_audio_path and Path(tts_audio_path).exists():
                await self._ffmpeg_merge(raw_video, tts_audio_path, output_mp4)
            else:
                await self._ffmpeg_convert(raw_video, output_mp4)
    ...
    @staticmethod
    async def _ffmpeg_merge(video: str, audio: str, output: str) -> None:
    ...
    @staticmethod
    async def _ffmpeg_convert(video: str, output: str) -> None:
        """Convert webm → mp4."""
        proc = await asyncio.create_subprocess_exec(
            "ffmpeg", "-y",
            "-i", video,

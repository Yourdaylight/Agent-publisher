...
async def test_verify_and_me_for_email_token(async_client):
    login_response = await async_client.post('/api/auth/login', json={'email': 'zhenghuizli@tencent.com'})
...
async def test_verify_invalid_token_rejected(async_client):
    response = await async_client.get('/api/auth/verify', headers={'Authorization': 'Bearer invalid-token'})
    assert response.status_code == 401
